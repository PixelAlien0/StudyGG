...
for (let attempt = 0; attempt < 30; attempt += 1) {
  if (await evaluate('document.readyState === "complete" && Boolean(window.STUDY_DATA)')) break;
  await new Promise((resolve) => setTimeout(resolve, 200));
}

const checks = [];
checks.push(['title', (await evaluate('document.title')).includes('Computer Networks I')]);
checks.push(['curated data counts', await evaluate('window.STUDY_DATA.slides.length === 113 && window.STUDY_DATA.qa.length === window.STUDY_DATA.meta.qaCount && window.STUDY_DATA.mcq.length === window.STUDY_DATA.meta.mcqCount && window.STUDY_DATA.qa.length >= 150 && window.STUDY_DATA.mcq.length >= 120 && window.STUDY_DATA.meta.curated')]);
...
await evaluate(`document.querySelector("[data-view-link=notes]").click(); const input=document.querySelector("#notes-search"); input.value="router"; input.dispatchEvent(new Event("input", {bubbles:true})); true`);
